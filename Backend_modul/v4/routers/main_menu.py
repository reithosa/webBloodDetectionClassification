...
from v4.core.queries import last_six_image, upload_image, upload_labels, get_last_image_indx
from v4.ml.inference import search_cells

main_router = APIRouter(tags=['Frontend'])
templates = Jinja2Templates(directory=TEMPLATES_PATH)

def image_analysys(path, name, confidence, engine):
    try:
        print("image_analysys START")
        result = search_cells(confidence, path)
...
async def post_image(image: UploadFile = File(...), confidence: float = Form(...)):
    engine = conn()
    img = image.file
    ...
    if image_analysys(img_path, img_name, confidence, engine):
